...
- **输入内容**：用户文本 `text`、语音转写 `voiceText`、图片输入说明，以及可选主色。
...
- 默认模型以 `backend/generation_settings.py` 为准，可用 `GEMINI_MODEL` 覆盖。
...
- 已新增 `POST /generate-diary`，包含 request/response schema、prompt builder、online Gemma client 和 fallback。
